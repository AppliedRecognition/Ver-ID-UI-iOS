...
struct SessionSettingsShareItem<T: VerIDSessionSettings>: Encodable {
    ...
    func encode(to encoder: Encoder) throws {
        ...
        try container.encode(settings.expiryTime, forKey: .expiryTime)
        try container.encode(settings.numberOfResultsToCollect, forKey: .numberOfResultsToCollect)
        ...
        try container.encode(settings.faceBoundsFraction.width, forKey: .faceWidthFraction)
        try container.encode(settings.faceBoundsFraction.height, forKey: .faceHeightFraction)
        try container.encode(settings.pauseDuration, forKey: .pauseDuration)
        try container.encode(settings.faceBufferSize, forKey: .faceBufferSize)
        try container.encode(settings.includeFaceTemplatesInResult, forKey: .extractFaceTemplates)
    }
}
