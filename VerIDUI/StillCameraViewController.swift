...
@objc open class StillCameraViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    
    private let captureSession = AVCaptureSession()
    let captureSessionQueue = DispatchQueue(label: "com.appliedrec.avcapture", attributes: [], target: nil)
    private var cameraInput: AVCaptureDeviceInput!
    private var setupResult: SessionSetupResult = .success
    private(set) var isSessionRunning = false {
        didSet {
            if oldValue != isSessionRunning && isSessionRunning {
                DispatchQueue.main.async { [weak self] in
                    self?.captureSessionStarted()
                }
            } else if oldValue != isSessionRunning {
                DispatchQueue.main.async { [weak self] in
                    self?.captureSessionEnded()
                }
            }
        }
    }
    private var waitingOnFocus = false
    private(set) internal var cameraPreviewView: CameraPreviewView!
    
    @objc lazy var operationQueue: OperationQueue = {
    ...
    private var observeSession: Bool = false {
        didSet {
            if oldValue != observeSession && observeSession {
                ...
                self.captureDevice.addObserver(self, forKeyPath: "adjustingFocus", options: .new, context: &self.adjustingFocusObserveContext)
    ...
    var requestedVideoData: Bool = false
        ...
        coordinator.animateAlongsideTransition(in: self.view, animation: nil, completion: { context in
            if !context.isCancelled {
                ...
                if let preview = self.cameraPreviewView {
                    ...
                    preview.videoPreviewLayer.videoGravity = AVLayerVideoGravity(rawValue: self.videoGravity)
                }
        ...
        do {
            ...
            if self.captureSession.canAddInput(videoDeviceInput) {
                ...
                DispatchQueue.main.async {
                    self.cameraPreviewView.videoPreviewLayer.videoGravity = AVLayerVideoGravity(rawValue: self.videoGravity)
...
//        self.videoDataOutput.videoSettings = [((kCVPixelBufferPixelFormatTypeKey as NSString) as String): kCVPixelFormatType_32BGRA]
    ...
    func captureSessionEnded() {
        
    }
    
    func captureSessionInterrupted() {
        self.cameraPreviewView.isHidden = true
    }
    
    func resumeInterruptedCaptureSession() {
        self.captureSessionQueue.async { [weak self] in
            guard let `self` = self else {
                return
            }
    ...
    func cameraBecameUnavailable(reason: String) {
        self.cameraPreviewView.isHidden = true
    }
    
    func cameraBecameAvailable() {
        self.cameraPreviewView.isHidden = false
    }
    
    func didFinishImageCapture(image: UIImage, highQuality: Bool) {
    ...
    func captureStillImage() {
        ...
        self.stillImageOutput.connection(with: AVMediaType.video)?.videoOrientation = videoPreviewLayerOrientation!
        self.waitingOnFocus = self.captureDevice.isAdjustingFocus
        if !self.waitingOnFocus {
            captureStillImageAfterFocus()
        }
    }
    
    private func captureStillImageAfterFocus() {
        let connection = self.stillImageOutput.connection(with: AVMediaType.video)
        self.stillImageOutput.captureStillImageAsynchronously(from: connection!) { [weak self] (sampleBuffer, error) in
            ...
            self.operationQueue.addOperation { [weak self] in
                ...
                if let uiImage = UIImage(data: data!, scale: self.stillImageScale) {
                    self.didFinishImageCapture(image: uiImage, highQuality: true)
                }
            }
        }
    }
    
    func captureVideoData() {
        self.captureSessionQueue.async {
            if !self.requestedVideoData {
                self.requestedVideoData = true
            }
        }
    }
    ...
    private var adjustingFocusObserveContext = 1
...
//                self.livePhotoModeButton.isHidden = !(isSessionRunning && isLivePhotoCaptureSupported)
        ...
        } else if context == &self.adjustingFocusObserveContext {
            if self.waitingOnFocus, let focusing = (change?[.newKey] as? NSNumber)?.intValue, focusing == 0 {
                self.waitingOnFocus = false
                self.captureStillImageAfterFocus()
            }
        } else {
